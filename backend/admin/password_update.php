...
if (isset($_POST['update_password'])) {
    ...
    // Check if ad_email exists using prepared statement
    $check_email_query = "SELECT ad_email FROM his_admin WHERE ad_email = ? LIMIT 1";
    ...
    if ($stmt->num_rows > 0) {
        if ($ad_pwd == $confirm_password) {
            // Update the password in the database using prepared statement
            $update_password_query = "UPDATE his_admin SET ad_pwd=? WHERE ad_email=? LIMIT 1";
            $stmt_update = $mysqli->prepare($update_password_query);
            $stmt_update->bind_param('ss', $ad_pwd, $ad_email);
            ...
            if ($update_password_run) {
                ...
                header("Location: password_update.php?ad_email=$ad_email");
                exit();
            }
        } else {
...
<body>
            ...
            <div class="login-container">
                <form method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input style="background-color: #bdded4;" type="text" id="ad_email" name="ad_email" value="<?php if (isset($_GET['ad_email'])) { echo htmlspecialchars($_GET['ad_email']);} ?>" readonly required>
                    </div>

                    <div class="form-group">
                        <label for="ad_pwd">New Password:</label>
                        <input type="password" id="ad_pwd" name="ad_pwd" placeholder="Enter your new password" required>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const password = document.getElementById('ad_pwd');
