...
if (isset($_POST['add_patient'])) {
    ...
    // SQL to insert captured values into his_surgery
    $query = "INSERT INTO his_surgery (s_pat_number, s_number, s_pat_name, s_pat_status, s_pat_ailment, s_doc) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param('ssssss', $s_pat_number, $s_number, $s_pat_name, $s_pat_status, $s_pat_ailment, $s_doc);
    $stmt->execute();

    // Check if insertion was successful
    if ($stmt) {
        // Update surgery_status in his_patients table
        $update_query = "UPDATE his_patients SET surgery_status = 'Admit' WHERE pat_number = ?";
        $update_stmt = $mysqli->prepare($update_query);
        $update_stmt->bind_param('s', $s_pat_number);
        $update_stmt->execute();

        if ($update_stmt) {
            $success = "Surgery Patient Added and Surgery Status Updated";
        } else {
            $err = "Surgery Patient Added but Failed to Update Surgery Status";
        }
    } else {
                                            ...
                                            <div class="form-row">
                                                ...
                                                <div class="form-group col-md-6">
                                                    ...
                                                    <select id="inputState" required="required" name="s_doc" class="form-control">
                                                        ...
                                                        // SQL code to get doctors randomly
                                                        $stmt = $mysqli->prepare($ret);
                                                        $stmt->execute(); // OK
                                                        $res = $stmt->get_result();
                                                        while ($row = $res->fetch_object()) {
                                                        ?>
                                                            <option value="<?php echo $row->doc_fname . ' ' . $row->doc_lname; ?>">
