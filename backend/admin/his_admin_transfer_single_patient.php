...
<?php
    ...
    if (isset($_POST['transfer_patient'])) {
        ...
        // SQL to insert captured values into his_patient_transfers
        ...
        if ($stmt->affected_rows > 0) {
            // SQL to update the discharge status in his_patients
            $update_query = "UPDATE his_patients SET pat_discharge_status = 'Discharged' WHERE pat_number = ?";
            $update_stmt = $mysqli->prepare($update_query);
            $update_stmt->bind_param('s', $t_pat_number);
            $update_stmt->execute();
    
            if ($update_stmt->affected_rows > 0) {
                $success = "Patient Transferred and Discharged.";
                
            } else {
                $err = "Patient transfer successful, but discharge status update failed.";
            }
            $update_stmt->close();
        } else {
