...
if (isset($_POST['update_password'])) {
    ...
    // Check if pat_email exists using prepared statement
    $check_email_query = "SELECT pat_email FROM his_patients WHERE pat_email = ? LIMIT 1";
    ...
    if ($stmt->num_rows > 0) {
        if ($pat_pwd == $confirm_password) {
            // Update the password in the database using prepared statement
            $update_password_query = "UPDATE his_patients SET pat_pwd=? WHERE pat_email=? LIMIT 1";
            $stmt_update = $mysqli->prepare($update_password_query);
            $stmt_update->bind_param('ss', $pat_pwd, $pat_email);
            ...
            if ($update_password_run) {
                ...
                header("Location: pat_password_update.php?pat_email=$pat_email");
                exit();
            }
        } else {
            ...
            <div class="login-container">
                <form method="POST" id="loginForm">
                    ...
                    <div class="form-group">
                        <label for="pat_email">Email address:</label>
                        <input style="background-color: #bdded4;" type="text" id="pat_email" name="pat_email" value="<?php if (isset($_GET['pat_email'])) {
                                                                                                                            echo htmlspecialchars($_GET['pat_email']);
                                                                                                                        } ?>" readonly required>
                    </div>

                    <div class="form-group">
                        <label for="pat_pwd">New Password:</label>
                        <input type="password" id="pat_pwd" name="pat_pwd" placeholder="Enter your new password" required>
                    ...
                    <div class="col-12 text-center">
                        <p><a href="pat_login.php" class="text-white-50 ml-1">Back to login</a></p>
                    </div>
                </form>
            </div>
        </div>
